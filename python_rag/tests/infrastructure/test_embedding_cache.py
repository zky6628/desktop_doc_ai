...
def test_put_and_get_roundtrip(tmp_path):
    ...
    try:
        ...
        repo.put_many("text-embedding-v4", 4, [("hash-a", vector)])

        hits = repo.get_many("text-embedding-v4", ["hash-a", "hash-miss"])
...
def test_cache_isolated_by_model(tmp_path):
    ...
    conn, repo = _repo(tmp_path)
    try:
        repo.put_many("model-a", 2, [("hash-x", [1.0, 2.0])])

        assert repo.get_many("model-b", ["hash-x"]) == {}
        assert "hash-x" in repo.get_many("model-a", ["hash-x"])
    finally:
        conn.close()
...
def test_put_many_is_idempotent(tmp_path):
    ...
    conn, repo = _repo(tmp_path)
    try:
        repo.put_many("m", 2, [("h", [1.0, 1.0])])
        repo.put_many("m", 2, [("h", [9.0, 9.0])])

        assert repo.get_many("m", ["h"])["h"] == pytest.approx([1.0, 1.0])
    finally:
        conn.close()
...
def test_corrupted_row_treated_as_miss(tmp_path):
    ...
    conn, repo = _repo(tmp_path)
    try:
        repo.put_many("m", 4, [("good", [1.0, 2.0, 3.0, 4.0])])
        # 直接写损坏行：blob 长度与维度不符
        conn.execute(
            "INSERT INTO embedding_cache"
            " (model_name, content_hash, vector, dimensions, created_at)"
            " VALUES (?, ?, ?, ?, ?)",
            ("m", "bad", b"\x00\x00\x00", 4, "2026-01-01T00:00:00+00:00"),
        )

        hits = repo.get_many("m", ["good", "bad"])
