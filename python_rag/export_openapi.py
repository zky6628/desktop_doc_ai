...
写入 python_rag/openapi-baseline.json 作为基线快照（与脚本同目录、
受版本控制，API 变更会在 git diff 中现形）。

用法（在 python_rag 目录下执行）：
    python export_openapi.py

校验方式：API 变更后重新执行本脚本，用 git diff 检查
openapi-baseline.json 的差异是否与本次 API 变更一致。
"""
import json
import os
import sys
...
OUT_PATH = os.path.join(BASE_DIR, "openapi-baseline.json")
